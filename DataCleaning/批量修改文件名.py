...
def rename(img_folder):
    for img_name in os.listdir(img_folder):  # os.listdir()： 列出路径下所有的文件
        ...
        dst = os.path.join(img_folder, "light_1027"+str(num)+".jpg") #dst： 修改后的目录名      注意此处str(num)将num转化为字符串,继而拼接
        num= num+1
...
def main():
    # for i in range(1,3):
        ...
        img_folder0 = r'E:\datasets\SecurityDatasets\light\JPEGImages' #图片的文件夹路径    直接放你的文件夹路径即可
        rename(img_folder0)
